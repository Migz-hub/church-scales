...
-- ============================================================
-- Phase 1: Full schema + RLS to replace localStorage mock backend
-- ============================================================
...
-- Security primitives (SECURITY DEFINER to avoid recursive RLS)
...
as $$
  select exists (
    ...
    where user_id = _user_id and ministry_id = _ministry_id and role = _role
  );
$$;
...
as $$
  select exists (
    select 1 from public.user_roles
    where user_id = _user_id and ministry_id = _ministry_id and role = any(_roles)
  );
$$;

-- Lock down direct EXECUTE from anon/authenticated for these helpers (still callable from policies)
...
-- Ministry RLS
create policy "ministries_select_members"
...
create policy "ministries_update_admins"
...
create policy "ministries_delete_owner"
  on public.ministries for delete
  to authenticated
  using (public.has_role(auth.uid(), id, 'owner'));

-- ministry_members RLS
create policy "members_select_same_ministry"
  on public.ministry_members for select
  to authenticated
  using (public.is_member_of(auth.uid(), ministry_id));
...
create policy "members_insert_self_or_admin"
  ...
  with check (
    user_id = auth.uid()
    or public.has_any_role(auth.uid(), ministry_id, array['owner','admin']::public.app_role[])
  );
...
-- user_roles RLS
create policy "roles_select_same_ministry"
  on public.user_roles for select
  to authenticated
  using (public.is_member_of(auth.uid(), ministry_id));

create policy "roles_insert_admins_or_self_initial"
  on public.user_roles for insert
  to authenticated
  with check (
    public.has_any_role(auth.uid(), ministry_id, array['owner','admin']::public.app_role[])
    -- Allow initial owner self-grant when a brand-new ministry has no roles yet
    or (
      user_id = auth.uid()
      and role = 'owner'
      and not exists (select 1 from public.user_roles ur where ur.ministry_id = user_roles.ministry_id)
    )
    -- Allow self-insert of 'member' role (used after joining via invite)
    or (
      user_id = auth.uid()
      and role = 'member'
    )
...
create policy "roles_delete_admins_or_self"
...
-- shared policies for ministry-scoped tables
create policy "functions_select_members" on public.ministry_functions for select to authenticated
  using (public.is_member_of(auth.uid(), ministry_id));
create policy "functions_write_admins" on public.ministry_functions for all to authenticated
  using (public.has_any_role(auth.uid(), ministry_id, array['owner','admin']::public.app_role[]))
  with check (public.has_any_role(auth.uid(), ministry_id, array['owner','admin']::public.app_role[]));

create policy "teams_select_members" on public.ministry_teams for select to authenticated
  using (public.is_member_of(auth.uid(), ministry_id));
...
create policy "member_perms_select_members" on public.member_permissions for select to authenticated
  using (public.is_member_of(auth.uid(), ministry_id));
...
create policy "schedules_select_members" on public.schedules for select to authenticated
  using (public.is_member_of(auth.uid(), ministry_id));
...
create policy "assignments_delete_leaders" on public.schedule_assignments for delete to authenticated
  using (exists (select 1 from public.schedules s where s.id = schedule_id and public.has_any_role(auth.uid(), s.ministry_id, array['owner','admin','leader']::public.app_role[])));
-- Updates: leaders can edit anything; assignees can update their own status/attendance
...
create policy "notifications_update_own" on public.notifications for update to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());
-- Inserts: any authenticated user in the same ministry may notify another member
create policy "notifications_insert_member" on public.notifications for insert to authenticated
  with check (
    ministry_id is null
    or public.is_member_of(auth.uid(), ministry_id)
  );
...
create policy "announcement_reads_insert_own" on public.announcement_reads for insert to authenticated
  with check (user_id = auth.uid());
create policy "announcement_reads_delete_own" on public.announcement_reads for delete to authenticated
  using (user_id = auth.uid());
...
create policy "unavailabilities_select_members" on public.unavailabilities for select to authenticated
  using (public.is_member_of(auth.uid(), ministry_id));
...
create policy "history_select_members" on public.schedule_history for select to authenticated
  using (public.is_member_of(auth.uid(), ministry_id));
create policy "history_insert_members" on public.schedule_history for insert to authenticated
  with check (
    actor_id = auth.uid() and public.is_member_of(auth.uid(), ministry_id)
  );
